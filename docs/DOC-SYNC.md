...
| A user-facing string (label, button copy, toast, email / SMS template, error) | All three locale catalogs under `src/lib/i18n/catalogs/` (HE / EN / RU) in the same commit. No literals in components — the ESLint rule will block the build otherwise (Phase 7+) |
